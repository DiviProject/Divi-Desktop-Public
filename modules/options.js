...
** strips --dev out of argv (double dash is not a divid argument) and returns
** {
**   dev: true,
**   testnet: true,
**   reindex: true,
...
exports.parse = function() {
  ...
  if (process.argv[0].match(/[Ee]lectron/)) {
    ...
    process.argv = process.argv.splice(1); /* striping /path/to/divi from argv */
  }
  ...
  args.map((arg, index) => {
    ...
    if (nDashes === 2) { /* double-dash: desktop-only argument */
      // delete param, so it doesn't get passed to divi-core
