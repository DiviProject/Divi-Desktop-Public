export class Amount {
  ...
  public getFractionalPart(): string {
    if (this.ifDotExist()) {
      // + causes the removal of 0's so leave in string form
      const dotAmount = (this.getAmount().toString()).split('.')[1];
      let compare = '';
      for (let i = 0; i < this.maxRoundingDigits; i++) {
        compare += '0';
      }
      if (dotAmount === compare) {
        return '';
      } else {
        return dotAmount;
      }
...
export class DateFormatter {
  ...
  public static format(date: Date): string {
    return new DateFormatter(date).dateFormatter();
  }
